...
// NewGencsrOutput creates a new instance of GencsrOutput
func NewGencsrOutput(details *ResultDetails, data json.RawMessage) (*GencsrOutput, error) {
	output := &GencsrOutput{
...
// NewGencsrResult creates a new instance of GencsrResult
func NewGencsrResult(ddl *agent.DDL) *GencsrResult {
	return &GencsrResult{ddl: ddl}
}
...
func (d *GencsrResult) RecordResult(output *GencsrOutput, reply *replyfmt.RPCReply) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *GencsrRequester) Do(ctx context.Context) (*GencsrResult, error) {
	dres := NewGencsrResult(d.r.client.ddl)
	...
	handler := func(pr protocol.Reply, r *rpcclient.RPCReply) {
		...
		output, err := NewGencsrOutput(NewResultDetails(pr.SenderID(), int(r.Statuscode), r.Statusmsg, pr.Time()), r.Data)
		if err != nil {
