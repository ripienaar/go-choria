...
// NewInfoOutput creates a new instance of InfoOutput
func NewInfoOutput(details *ResultDetails, data json.RawMessage) (*InfoOutput, error) {
	output := &InfoOutput{
...
// NewInfoResult creates a new instance of InfoResult
func NewInfoResult(ddl *agent.DDL) *InfoResult {
	return &InfoResult{ddl: ddl}
}
...
func (d *InfoResult) RecordResult(output *InfoOutput, reply *replyfmt.RPCReply) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *InfoRequester) Do(ctx context.Context) (*InfoResult, error) {
	dres := NewInfoResult(d.r.client.ddl)
	...
	handler := func(pr protocol.Reply, r *rpcclient.RPCReply) {
		...
		output, err := NewInfoOutput(NewResultDetails(pr.SenderID(), int(r.Statuscode), r.Statusmsg, pr.Time()), r.Data)
		if err != nil {
